...
The `tool` method returns the installation path of to the SonarScanner. `withSonarQubeEnv` retrieves information about the SonarQube server, which then does not need to be specified via parameters for the SonarScanner.
Since the Topaz for Total Test plugin creates one result file per test scenario, the location and name of all these files need to be passed to the SonarScanner via the `sonar.testExecutionReportPaths` parameter. The value will have to be a comma separated list of path and file names which is being built in the initial stage.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQ0OTQ1OTk1LC0yNDI3ODk5MjcsLTIwNj
U3NDcyNzYsLTE3MDYzNjc2NzIsLTE2NjUxODU5NjZdfQ==
-->