...
The results can be queried by the `status` property. If the status is not okay, i.e. the quality gate failed, the assignments corresponding to the set will be regressed using the [`ISPW operation plugin`](https://plugins.jenkins.io/compuware-ispw-operations/), an email is sent to the owner of the set, and the pipeline being aborted using the `error` method.
