...
* Press SAVE.
We have now configured the build step for running Total Test for functional testing. We can also add a post build step that allows us to see the result as JUnit output - a often used view of test results in Jenkins. See a section below for how to do this.
...
* The same fields are available as for setting up the freeform project. See above for details on each field. Fill the fields as illustrated below
...
*Enter `TTTReport/*.xml` in the Test reports XML file. This holds the Total Test result files. If the report output folder has been customized in the pipeline, that value must be used here instead.
* Press SAVE.
...
* Gather the information for the required fields from the Topaz for Total Test project and enter it in the fields:
  ...
  * Press SAVE.
We have now configured the build step for running Total Test. We can also add a post build step that allows us to see the result as JUnit output - a often used view of test results in Jenkins. See the chapter below for how to do this.
