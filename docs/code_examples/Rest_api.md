...
user is not enabled. An Functional Test administrator must then assign a license to the server.
#### Example request
```
POST /authenticate
...
- GET `/<resourceName>`
Following URL parameters can be used
...
Here you can see that the componentName and environmentId parameters are required while there are
optional parameters. Clicking the Try it out button will execute the API and you will see the request
...
The example request contains a callback URL to https://demo.xatester.com/publish/demo. The result is sent to this URL through a POST request after the execution finishes
