...
4. Or if you prefer to use GitHub action **ispw-sync-local** , you may specify the **sync** step as below, the workflow is broken up into the following stages ([Download a template - ispw-sync-local-generate-deploy.yml](https://github.com/bmc-compuware/ispw-deploy/blob/main/.github/templates/ispw-sync-local-generate-deploy.yml):
   * Checkout - checks out the source code.
